...
const EventReportPage: React.FC = () => {
  ...
  const fetchEventReportData = async (filters: any, paginationState: any, sortingState: any) => {
    if (!filters) return;
    setIsLoading(true);
    try {
      ...
      // Change endpoint to event report
      ...
      const initialTransformed = dataArray.map((item: any, index: number) => ({
        ...
        notification: item.notification || item.eventType || item.alertType || "Event",
        location: "Loading...",
        message: item.message || item.description || item.details || "",
        dateTime: item.dateTime || item.timestamp || item.createdAt,
        coordinates: item.coordinates || item.location || item.lat + "," + item.lng || "21.991253888888888, 78.92976777777777",
      }));
  ...
  const handleFilterSubmit = async (filters: any) => {
    ...
    setShowTable(true);
    await fetchEventReportData(filters, { pageIndex: 0, pageSize: 10 }, []);
  };
  ...
  return (
    <div>
      ...
      <h1 className="text-xl font-bold mb-4">Event Reports</h1>
      <ReportFilter
