...
export default function RoutesMaster() {
  ...
  useEffect(() => {
    ...
    if (r === "branchgroup" && decoded?.AssignedBranch?.length) {
      ...
      setFilters((prev) => ({
        ...prev,
        branch: decoded.AssignedBranch[0]._id,
      }));
    }
  }, []);
  ...
  const enrichedDeviceItems = useMemo(() => {
    ...
    const deviceId =
      typeof editTarget.deviceObjId === "object"
    ...
    return [
      {
        label:
          (typeof editTarget.deviceObjId === "object" &&
            (editTarget.deviceObjId.deviceName ||
              editTarget.deviceObjId.name)) ||
          "Device",
        value: deviceId,
      },
      ...deviceItems,
  ...
  const getDeviceName = useCallback(
    (route: Route) => {
      ...
      if (typeof route.deviceObjId === "object") {
        return (
          route.deviceObjId.deviceName || route.deviceObjId.name || "Device"
        );
      }
  ...
  const filterBranchOptions = useMemo(() => {
    ...
    if (filters.school) {
      return branchData
        .filter((b: any) => {
          const schoolId =
            typeof b.schoolId === "object" ? b.schoolId?._id : b.schoolId;
  ...
  const activeFilters = useMemo(() => {
    ...
    if (filters.school && isSuperAdmin) {
      ...
      active.push({
        key: "school",
        label: "School",
        value: schoolObj?.label || filters.school,
      });
    }
    if (filters.branch && !isBranchRole) {
      const branchObj = filterBranchOptions.find(
        (b) => b.value === filters.branch
      );
      active.push({
        key: "branch",
        label: "Branch",
        value: branchObj?.label || filters.branch,
      });
    }
    return active;
        ...
        ? [
            {
              ...
              accessorFn: (row: Route) => {
                ...
                const s = schoolData?.find(
                  (sc: any) => sc._id === row.schoolId
                );
  ...
  const updateRouteMutation = useMutation({
    mutationFn: async ({ routeId, data }: any) =>
      api.put(`/route/${routeId}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["routes"] });
  ...
  const handleSave = (updatedData: Partial<Route>) => {
    ...
    for (const key in updatedData) {
      ...
      if (key === "schoolId" || key === "branchId" || key === "deviceObjId") {
        if (typeof originalVal === "object" && originalVal)
          originalVal = (originalVal as any)._id;
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (isBranchRole) selectedBranch = decoded?.id || userBranchId || "";
      ...
      selectedBranch =
        branch ||
        (decoded?.AssignedBranch?.length ? decoded.AssignedBranch[0]._id : "");
  ...
  const { table, tableElement } = CustomTableServerSidePagination({
    ...
    emptyMessage:
      isSearchActive || isFilterActive
        ? "No routes found matching your search/filter criteria"
        : "No routes found",
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(isSuperAdmin || isSchoolRole || isBranchGroupRole) && (
                    <div className="grid gap-2">
                      <Label>Branch *</Label>
                      <Combobox
                        items={addFormBranchOptions}
                        value={branch}
                        onValueChange={setBranch}
                        placeholder={
                          isSchoolRole || isBranchGroupRole
                            ? "Select branch..."
                            : !school
                            ? "Select school first"
                            : "Select branch..."
        ...
        <div className="flex justify-center items-center">
          <div className="w-[350px] max-w-[350px]">
            <Alert<Route>
              ...
              description={`This will permanently delete route ${
                deleteTarget?.routeNumber || "this route"
      ...
      {editTarget && (
        <DynamicEditDialog
          data={{
            ...editTarget,
            schoolId:
              typeof editTarget.schoolId === "object"
                ? editTarget.schoolId?._id
                : editTarget.schoolId,
            branchId:
              typeof editTarget.branchId === "object"
          ...
          onSave={handleSave}
          fields={getRouteFieldConfigs(
            schoolOptions,
            addFormBranchOptions,
            enrichedDeviceItems
          ).filter((field) => {
            if (field.key === "schoolId" && (isSchoolRole || isBranchRole))
              return false;
